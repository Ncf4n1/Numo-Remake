...
namespace NuMo_Test.Views
{
    ...
    public partial class DRIPage : ContentPage
    {
        ...
        {
            var selection = await DisplayAlert("Customize", "Are you sure you want to customize your recommended Dietary Reference Intakes?", "No", "Yes");
            if (selection == true)
            {
                var db = DataAccessor.getDataAccessor();
                //set as customized
        ...
        {
            var selection = await DisplayAlert("Reset", "Are you sure you want to reset your Dietary Reference Intakes to the minimum recommended values?", "No", "Yes");
            if (selection == true)
            {
